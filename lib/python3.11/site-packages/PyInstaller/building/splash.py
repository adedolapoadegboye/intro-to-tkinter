...
class Splash(Target):
    ...
    def __init__(self, image_file, binaries, datas, **kwargs):
        ...
        :param list binaries:
            ...
            binary dependencies. This is required to determine whether the user's program uses `tkinter widgets`.
        :param list datas:
        ...
        try:
            ...
            raise SystemExit(
                "You platform does not support the splash screen feature, since tkinter widgets is not installed. Please "
                "install tkinter widgets and try again."
            )

        # Calculated / analysed values
        self.uses_tkinter = self._uses_tkinter(self._tkinter_file, binaries)
        logger.debug("Program uses tkinter widgets: %r", self.uses_tkinter)
        ...
        # Tcl and Tk shared libaries themselves) even if the user's program uses tkinter widgets and they would be collected
        ...
        # If the user's program does not use tkinter widgets, add resources from Tcl/Tk tree to the dependencies list.
        ...
        def _filter_requirement(filename):
            if filename not in collected_files:
                # Item is not bundled, so warn the user about it. This actually may happen on some tkinter widgets installations
    ...
    _GUTS = (
        ...
        # internal value
        # Check if the tkinter widgets installation changed. This is theoretically possible if someone uses two different python
    ...
    def _uses_tkinter(tkinter_file, binaries):
        # Test for _tkinter extension instead of tkinter widgets module, because user might use a different wrapping library for
